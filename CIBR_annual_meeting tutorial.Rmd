...
1. Introduction to new function in FLAREr that will generate tabular output (csv) that is more easily accessable via cloud storage. 
...
1) First you need to establish the connection with the remote server
...
2) You need to open the dataset.  This only discovers the specific columns and column types of the dataset and does not actually read in the data yet
...
3) You need to `collect()` the data. Subsetting the data through filters, selects, and summaries can help speed up the transfer of data. 
...
And one with all ensemble member from all three LER models (GLM, GOTM, and Simstrat)
...
The site and depth are a combined variable in the score because each depth is considered a separate site when joining the targets and forecasts.  Here we separate them back out.
